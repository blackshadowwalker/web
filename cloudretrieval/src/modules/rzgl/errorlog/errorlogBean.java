 ...
 */
public class errorlogBean extends ResourceClose
{
     ...
     * 根据条件语句查询信息AJAX
     */
    public String queryTSErrorlog(int perPage,int curPage,String s,ServletContext sc,HttpSession session)
        throws CustomException,Exception
    {
        ...
        try
        {
            ...
            sqlSB.append("select id,user_code,user_name,user_ip,user_time,user_nr from t_s_cwlog ");
            if(s!=null&&!s.equals("")){
            	sqlSB.append(" ");
            	sqlSB.append(s);
            }
            //sqlSB.append(" order by id desc ");
            countSB.append("select count(1)  from  t_s_cwlog where status=1 " );
            ...
            while (rs!=null && rs.next())
            {
                ...
                {
                	if(i==6){
                		infoSB.append(basePublic.out(rs.getString(i).replace("'", "‘")) + "</C>");
                	}else{
            ...
            LogBean.wirte_log(((DataSource) sc.getAttribute("basedb")).getConnection(), "t_s_czlog", bform.user_code + "</C>"+ bform.user_name + "</C>" + bform.clientIP + "</C>"+ "查询异常日志信息成功！",errorlogBean.class.getName()); 
        }
        catch (SQLException sql)
		{
			...
			LogBean.wirte_log(((DataSource) sc.getAttribute("basedb"))
					...
					+ "errorlogBean.queryTSErrorlog出现数据库连接错误,操作为：查询异常日志信息失败!" + sql.getMessage(),errorlogBean.class.getName());
		...
		catch (UnsupportedEncodingException ue)
		{
			...
			LogBean.wirte_log(((DataSource) sc.getAttribute("basedb"))
					.getConnection(), "t_s_cwlog", bform.user_code + "</C>"
					+ bform.user_name + "</C>" + bform.clientIP + "</C>"
					+ "errorlogBean.queryTSErrorlog出现转化字符错误,操作为：查询异常日志信息失败!" + ue.getMessage(),errorlogBean.class.getName());
		...
		catch (Exception ce)
		{
			...
			LogBean.wirte_log(((DataSource) sc.getAttribute("basedb"))
					.getConnection(), "t_s_cwlog", bform.user_code + "</C>"
					+ bform.user_name + "</C>" + bform.clientIP + "</C>"
					+ "errorlogBean.queryTSErrorlog出现错误,操作为：查询异常日志信息失败!" + ce.getMessage(),errorlogBean.class.getName());
    ...
    public String detailTSErrorlog(String id,ServletContext sc,HttpSession session)
    throws CustomException,Exception
{
    ...
    try
    {
        ...
        sqlSB.append("select user_code,user_name,user_ip,user_time,user_nr from t_s_cwlog where id=?");
        ...
        LogBean.wirte_log(((DataSource) sc.getAttribute("basedb")).getConnection(), "t_s_czlog", bform.user_code + "</C>"+ bform.user_name + "</C>" + bform.clientIP + "</C>"+ "查询异常日志详细信息成功！",errorlogBean.class.getName());
        return infoSB.toString();
    }
    catch (SQLException sql)
	{
		...
		LogBean.wirte_log(((DataSource) sc.getAttribute("basedb"))
				.getConnection(), "t_s_cwlog", bform.user_code + "</C>"
				+ bform.user_name + "</C>" + bform.clientIP + "</C>"
				+ "errorlogBean.detailTSErrorlog出现数据库连接错误,操作为：查询异常日志详细信息失败!" + sql.getMessage(),errorlogBean.class.getName());
		throw new CustomException((rb.getString("exp.databaseConnectError")));//出现数据库连接方面的错误
	}
	catch (Exception ce)
	{
		...
		LogBean.wirte_log(((DataSource) sc.getAttribute("basedb"))
				.getConnection(), "t_s_cwlog", bform.user_code + "</C>"
				+ bform.user_name + "</C>" + bform.clientIP + "</C>"
				+ "errorlogBean.detailTSErrorlog出现错误,操作为：查询异常日志详细信息失败!" + ce.getMessage(),errorlogBean.class.getName());
		throw new CustomException((rb.getString("exp.unknownError")));//出现未知异常错误
	}
    finally
    {
