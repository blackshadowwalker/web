...
var gd = document.body.clientHeight-200;
	tObject.init({
				reportTitle : '<bean:message key="user.title" bundle="${language}"/>',
				...
				cols : new ColsObject({
							...
							viewLineNum : true,
							viewTotalRow : 100,
							callbackDblClick: dblcallback,
							cols : [{
										name : '<bean:message key="user.columns.id" bundle="${language}"/>',   //内部编号
										alias : 'id',
										width : 100,
										hide : true
									}, {
										name : '<bean:message key="user.columns.userCode" bundle="${language}"/>',    //用户代码
										alias : 'user_code',
										width : 100
									}, {
										name : '<bean:message key="user.columns.userName" bundle="${language}"/>',    //用户名称
										alias : 'user_name',
										width : 100
									}, {
										name : '<bean:message key="user.columns.userIP" bundle="${language}"/>',    //用户IP
										alias : 'user_ip',
										width : 100
									}, {
										name : '<bean:message key="user.columns.time" bundle="${language}"/>',     //时间
										alias : 'user_time',
										width : 180
									}, {
										name : '<bean:message key="user.columns.content" bundle="${language}"/>',     //内容
										alias : 'user_nr',
										width : 600
									}]
						}),
