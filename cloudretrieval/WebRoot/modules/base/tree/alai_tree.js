...
function alai_imagelist() {
	try {
		...
		this.add = function(src, key) {
			...
			if (src.indexOf("/") == -1)
				src = iml.path + src
			if (!(/\.gif$|\.jpg$|\.png$|\.bmp$/i).test(src))
				src += "." + iml.type
			if (key == null || key == "")
				key = src.replace(/(.*\/){0,}([^\.]+).*/ig, "$2")
			item[count] = img;
			img.index = count;
			item[key] = img;
			count++;
...
function alai_tree(imageList, indent, toObject, labelStyle) {
	try {
		if (!document.getElementById) {
			throw new Error(999,
					"Your browser dosen't support Alai_tree(),IE5.0+ is recommended.");
		}
		...
		var setExIcon = function(srcNode) {
			if (srcNode.hasChild && showExpand) {
				...
				if (srcNode == nodes[0])
					sEx = "_top";
				srcNode.exIcon.src = srcNode.expanded
						? icons["collapse" + sEx].src
						: icons["expand" + sEx].src;
				srcNode.icon.style.marginLeft = 0;
			} else {
				if (showBranch) {
					srcNode.exIcon.src = srcNode.next == null
							? icons["twig"].src
							: icons["leaf"].src
				} else if (showExpand) {
		...
		var doSelect = function(srcNode) {
			if (selectedNode != null) {
				selectedNode.label.style.background = "";
				selectedNode.label.style.color = "black";
			}
			srcNode.label.style.background = "highlight";
			srcNode.label.style.color = "highlighttext";
			selectedNode = srcNode
		}
		var doFocus = function(srcNode) {
			if (srcNode == selectedNode) {
				srcNode.label.style.background = "highlight";
				srcNode.label.style.color = "highlighttext";
			}
			activeNode = srcNode
		...
		var doBlur = function(srcNode) {
			if (srcNode == selectedNode) {
				srcNode.label.style.background = "buttonface";
				srcNode.label.style.color = "windowtext";
			}
			activeNode = null
		}
		...
		var validObject = function(cObject) {
				...
				case 38 :
					if (srcNode.prev == null) {
						if (srcNode.parent != root) {
							srcNode.parent.focus();
							srcNode.parent.select();
						}
					} else {
						var node = srcNode.prev;
						while (true) {
					...
					break;
				case 40 :
					if (srcNode.first != null && srcNode.expanded) {
						srcNode.first.focus();
						srcNode.first.select();
					} else {
						var node = srcNode;
						while (true) {
							if (node.next != null || node.parent == root)
								break;
							node = node.parent
						}
						if (node.next != null) {
		...
		var addNode = function(toNode, relation, text, key, ico, exeCategory,
				exeArg) {
			var node = {}
			...
			if (relation == null)
				relation = "last"
			if (toNode == root && (relation != "first" && relation != "last"))
				relation = "last"
			...
			switch (relation) {
				case "first" :
					...
					if (toNode.hasChild) {
						toNode.children[0].container.insertAdjacentElement(
								"beforeBegin", container);
					} else {
						toNode.container.insertAdjacentElement("beforeEnd",
								container)
					}
					node.parent = toNode
					toNode.children[toNode.children.length] = node
					if (toNode.hasChild) {
						node.next = toNode.first;
						toNode.first.prev = node;
						toNode.first = node;
					} else {
				...
				case "next" :
					tier = toNode.getTier()
					toNode.container.insertAdjacentElement("afterEnd",
							container)
				...
				case "prev" :
					tier = toNode.getTier()
					toNode.container.insertAdjacentElement("beforeBegin",
							container)
					node.parent = toNode.parent
					toNode.parent.children[toNode.parent.children.length] = node
					if (toNode.prev == null)
						node.parent.first = node
					node.prev = toNode.prev;
					node.next = toNode;
					toNode.prev = node
					break;
				default :
			...
			with (nbody) {
				...
				nbody.style.textIndent = indent * (tier - 1)
			}
			var icon = new Image()
			ico = ico == null || ico == "" ? "default" : ico
			if (typeof(ico) == "string")
				if (ico.toLowerCase().indexOf(".gif") != -1
						|| ico.toLowerCase().indexOf(".jpg") != -1) {
					ico = imageList.add(ico).index;
				}
			if (icons[ico] != null) {
			...
			label.style.cssText = labelStyle != null
					? labelStyle
					: "padding:1;margin-left:2;vertical-align:middle;text-align:left;"
			...
			with (label) {
				...
				/*
				 * ondblclick=function(){
				 * if(run(node.ondblclick))run(tree.ondblclick,node)}
				 * 
				 * onclick=function(){ return false;} ondblclick=function(){
				 * return false;}
				 ...
				 * onmousedown=function(){ alert("mouse
				 * down");if(run(node.onmousedown))run(tree.onmousedown,node)
				 * -->}
				 * onmouseup=function(){if(run(node.onmouseup))run(tree.onmouseup,node)}
				 * onkeypress=function(){ alert("keypress");
				 * if(run(node.onkeypress))run(tree.onkeypress,node)}
				 * onkeydown=function(){ alert("keydown");
				 * if(run(node.onkeydown)){if(run(tree.onkeydown,node))doKeyDown(node);}}
			...
			with (icon) {
				...
				/*
				 * onclick=function(){var color=node.label.style.color;
				 * run(node.select);if(run(node.onclick))if(run(tree.onclick,node))run(node.execute);node.label.style.color=color;}
			...
			nbody.onblur = function() {
				if (node.onfocus()) {
					if (tree.onfocus(node))
						doFocus(node);
				}
			}
			nbody.onfocus = function() {
				if (node.onblur()) {
					if (tree.onblur(node))
						doBlur(node);
				}
			...
			if (showLine) {
				...
				if (node.prev != null)
					setLine(node.prev, tier - 1)
				var n = node.parent;
				var i = tier - 2;
				while (n != root) {
					if (n.next != null)
						node.lineIcon[i].src = icons["line"].src;
					n = n.parent;
					i--
				}
			}
			...
			if (ma != root) {
				...
				setExIcon(ma);
				if (ma.exIcon) {
					ma.exIcon.onmousedown = function() {
						ma.expand();
					}
				}
			}
			...
			node.label = label
			node.icon = icon
			node.add = function(text, ico, exeType, exeArg) {
				return addNode(this, "last", text, "", ico, exeType, exeArg);
			}
			node.addLink = function(_url, text, target, img) {
				if (text == null || text == "")
					text = _url;
				var exeArg = "url";
				...
				if ((img == null || img == ""))
					img = "link";
				return addNode(node, "last", text, "", img, exeArg, _url)
			}
			node.expanded = true
			node.expand = function(isShow, incSub) {
				...
				setExIcon(node)
				if (node.expanded) {
					if (run(node.onexpand)) {
						run(tree.onexpand, node);
					}
				} else {
					if (node.oncollapse()) {
			...
			node.getPath = function(separator) {
				...
				for (var i = tier; i > 0; i--) {
					path = separator + oNode.label.innerText + path;
					oNode = oNode.parent;
				}
				return path;
			}
			node.getSibling = function() {
		...
		var remove = function(srcNode) {
			...
			if (ma.first == srcNode && ma.last == srcNode && ma != root) {
				ma.hasChild = false;
				document.createElement("div").insertAdjacentElement(
						"afterBegin", ma.exIcon);
				setExIcon(ma);
			...
			srcNode.container.removeNode(true);
			nodes.remove(srcNode.getIndex());
			count--;
			srcNode = null;
		}
		this.removeNode = function(srcNode) {
			if (!srcNode.hasChild)
				remove(srcNode);
			else {
				for (var i = 0; i < srcNode.children.length; i++) {
					tree.removeNode(srcNode.children[i]);
				}
				tree.removeNode(srcNode);
			}
		}
		root.add = function(text, ico, exeType, exeArg) {
			return addNode(root, "last", text, "", ico, exeType, exeArg);
		}
		root.addLink = function(_url, text, target, img) {
			if (text == null || text == "")
				text = _url;
			var exeArg = "url";
			if (target != null && target != "")
			...
			if ((img == null || img == ""))
				img = "link";
			return addNode(root, "last", text, "", img, exeArg, _url)
		}
		...
		this.expandAll = function(isShow) {
			isShow = isShow == null ? !root.expanded : isShow;
			for (var i = 0; i < count; i++)
				nodes[i].expand(isShow);
		...
		this.embed = function(obj, where) {
			...
			if (where == null)
				where = "afterBegin";
			var s = where.toLowerCase();
			if (s != "beforebegin" || s != "afterbegin" || s != "beforeend"
					|| s != "afterend")
				where = "afterBegin";
			obj.insertAdjacentElement(where, tree.body);
		}
		this.count = function() {
			return count;
		}
		...
		this.onclick = this.ondblclick = this.onmousemove = this.onmouseover = this.onmouseout = this.onmouseup = this.onmousedown = this.onkeypress = this.onkeydown = this.onkeyup = this.oncollapse = this.onexpand = this.afteradd = this.onblur = this.onfocus = this.onselect = this.oncheck = this.onview = new Function("return true;");
		this.add = addNode
		...
		this.pathParse = function(pathList, sort, ps, ls, ico) {
			...
			for (var i = 0; i < nodes.length; i++) {
				colPath[colPath.length] = nodes[i].getPath();
			}
			var pathIndex = function(path) {
				for (var i = colPath.length - 1; i >= 0; i--)
					if (colPath[i] == path)
						return i;
				return -1;
			}
			for (var i = 0; i < list.length; i++) {
				if (list[i] == "")
					continue;
				var path = list[i].split(ps), node = root;
