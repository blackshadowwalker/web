...
FCKXHtml._GetMainXmlString = function() {
	...
	return A.serializeToString(this.MainNode).replace(
...
FCK.InitializeBehaviors = function() {
	if (FCKConfig.ShowBorders) {
		var A = FCKTools.AppendStyleSheet(this.EditorDocument,
	...
	var C = function(e) {
		...
		if (e.ctrlKey && !e.shiftKey && !e.altKey) {
			switch (e.which) {
				...
				case 86 :
				case 118 :
					D = (FCK.Status != FCK_STATUS_COMPLETE || !FCK.Events
							.FireEvent("OnPaste"));
					break;
			};
...
FCK.MakeEditable = function() {
	try {
		...
		FCK.EditorDocument
				.execCommand('useCSS', false, !FCKConfig.GeckoUseSPAN);
		FCK.EditorDocument.execCommand('enableObjectResizing', false,
				!FCKConfig.DisableImageHandles);
		FCK.EditorDocument.execCommand('enableInlineTableEditing', false,
				!FCKConfig.DisableTableHandles);
	} catch (e) {
	};
};
FCK.Focus = function() {
...
FCK.SetHTML = function(A, B) {
	...
	if (B || FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
		...
		if (FCKConfig.FullPage && FCKRegexLib.BodyContents.test(A)) {
			...
			A = A
					.replace(
							FCKRegexLib.HeadCloser,
							'<link href="'
									+ FCKConfig.BasePath
									+ 'css/fck_internal.css'
									+ '" rel="stylesheet" type="text/css" _fcktemp="true" /></head>');
			...
			if (A.length == 0)
				...
				FCK.EditorDocument.body.innerHTML = A.replace(
						FCKRegexLib.TagBody, '>' + GECKO_BOGUS + '<');
			else
