...
if (FCKBrowserInfo.IsGecko) {
	function Window_OnResize() {
		...
		oFrame.height = 0;
		var oCell = document
				.getElementById(FCK.EditMode == FCK_EDITMODE_WYSIWYG
						? 'eWysiwygCell'
						: 'eSource');
...
function LoadPageConfig() {
	...
	if (FCKConfig.AllowQueryStringDebug
			&& (/fckdebug=true/i).test(window.top.location.search))
...
function LoadLanguageFile() {
	...
	FCKScriptLoader.AddScript('lang/' + FCKLanguageManager.ActiveLanguage.Code
			+ '.js');
};
function LoadEditor() {
	FCKScriptLoader.OnEmpty = null;
