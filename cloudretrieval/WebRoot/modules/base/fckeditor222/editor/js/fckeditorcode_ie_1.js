...
FCKTools.DisableSelection = function(A) {
	...
	while (e = A.all[i++]) {
		switch (e.tagName) {
			...
			case 'TEXTAREA' :
			case 'INPUT' :
			case 'SELECT' :/* Ignore the above tags */
				break;
			default :
				e.unselectable = 'on';
		};
...
if (FCKConfig.ProcessHTMLEntities) {
	...
	FCKXHtmlEntities.EntitiesRegex = new RegExp('[' + FCKXHtmlEntities.Chars
	...
	switch (B.nodeType) {
		case 1 :
			...
			if (!FCKRegexLib.ElementName.test(C))
				return false;
			C = C.toLowerCase();
			if (FCKBrowserInfo.IsGecko && C == 'br' && B.hasAttribute('type')
	...
	for (var n = 0; n < E.length; n++) {
		...
		if (F.specified) {
			...
			if (G.startsWith('_fck'))
				...
				H = B.style.cssText;
			else if (G == 'class' || G.indexOf('on') == 0)
				H = F.nodeValue;
			else if (D == 'body' && G == 'contenteditable')
				continue;
			else if (F.nodeValue === true)
				H = G;
			else if (!(H = B.getAttribute(G, 2)))
				H = F.nodeValue;
			if (FCKConfig.ForceSimpleAmpersand && H.replace)
				H = H.replace(/&/g, '___FCKAmp___');
...
FCKXHtml.TagProcessors['area'] = function(A, B) {
	if (!A.attributes.getNamedItem('coords')) {
		...
		if (C && C != '0,0,0')
			FCKXHtml._AppendAttribute(A, 'coords', C);
	};
	if (!A.attributes.getNamedItem('shape')) {
		var C = B.getAttribute('shape', 2);
		if (C && C.length > 0)
			FCKXHtml._AppendAttribute(A, 'shape', C);
	};
	return A;
};
...
if (!(FCKCodeFormatter = NS.FCKCodeFormatter)) {
	...
	FCKCodeFormatter._ProtectData = function(A, B, C, D) {
		return B + '___FCKpd___' + FCKCodeFormatter.ProtectedData.addItem(C)
				+ D;
	};
	FCKCodeFormatter.Format = function(A) {
		FCKCodeFormatter.ProtectedData = new Array();
		var B = A.replace(this.Regex.ProtectedTags,
...
FCKUndo.SaveUndoStep = function() {
	...
	if (FCKUndo.CurrentIndex >= 0
			&& A == FCKUndo.SavedData[FCKUndo.CurrentIndex][0])
		return;
...
FCK.GetXHTML = function(A) {
	...
	if (FCKConfig.FullPage)
		C = FCKXHtml.GetXHTML(
				this.EditorDocument.getElementsByTagName('html')[0], true, A);
	else {
...
FCK.ProtectUrls = function(A) {
	...
	A = A
			.replace(FCKRegexLib.ProtectUrlsImgApo,
					'$1$2$3$2 _fcksavedurl=$2$3$2');
...
FCKAnchorsProcessor.ProcessDocument = function(A) {
	...
	while (i >= 0 && (C = B[i--])) {
		if (C.name.length > 0
				&& (!C.getAttribute('href') || C.getAttribute('href').length == 0)) {
			var D = FCKDocumentProcessors_CreateFakeImage('FCK__Anchor', C
							.cloneNode(true));
...
FCKPageBreaksProcessor.ProcessDocument = function(A) {
	...
	while (i >= 0 && (C = B[i--])) {
		if (C.style.pageBreakAfter == 'always' && C.childNodes.length == 1
			...
			var D = FCKDocumentProcessors_CreateFakeImage('FCK__PageBreak', C
							.cloneNode(true));
			C.parentNode.insertBefore(D, C);
			C.parentNode.removeChild(C);
		};
...
FCK._BehaviorsStyle = '<style type="text/css" _fcktemp="true">\ INPUT { behavior:url('
		+ FCKConfig.FullBasePath + 'css/behaviors/hiddenfield.htc);} ';
if (FCKConfig.ShowBorders)
	FCK._BehaviorsStyle += 'TABLE { behavior: url(' + FCKConfig.FullBasePath
...
if (FCKConfig.DisableTableHandles)
	sNoHandlers += ', TABLE';
FCK._BehaviorsStyle += sNoHandlers + ' { behavior: url('
	...
	switch (e.keyCode) {
		case 13 :
			if (FCKConfig.UseBROnCarriageReturn
				...
				Doc_OnKeyDownUndo();
				if (FCK.EditorDocument.queryCommandState('InsertOrderedList')
						|| FCK.EditorDocument
...
FCK.SetHTML = function(A, B) {
	if (B || FCK.EditMode == FCK_EDITMODE_WYSIWYG) {
		...
		var C;
		if (FCKConfig.FullPage) {
			var C = FCK._BehaviorsStyle + '<link href="'
					+ FCKConfig.FullBasePath + 'css/fck_internal.css'
