...
FCKPanel.prototype.Show = function(x, y, A, B, C) {
	...
	if (this.IsRTL) {
		...
		if ((x + this.PanelDiv.offsetWidth) > this._Window.document.body.clientWidth)
			x -= x + this.PanelDiv.offsetWidth
					- this._Window.document.body.clientWidth;
	};
	if (x < 0)
		x = 0;
...
FCKTableHandler.GetSelectedCells = function() {
	...
	if (B.rangeCount == 1 && B.anchorNode.nodeType == 3) {
		var C = FCKTools.GetElementAscensor(B.anchorNode, 'TD');
		if (C) {
			A[0] = C;
			return A;
		};
...
var FCKToolbarBreak = function() {
	...
	A.style.clear = A.style.cssFloat = FCKLang.Dir == 'rtl' ? 'right' : 'left';
...
FCKDialog.Show = function(A, B, C, D, E, F, G) {
	...
	this.Window = K;
	try {
		window.top.captureEvents(Event.CLICK | Event.MOUSEDOWN | Event.MOUSEUP
				| Event.FOCUS);
