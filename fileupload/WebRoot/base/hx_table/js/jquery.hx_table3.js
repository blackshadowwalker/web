	...
	if (typeof(TableObject3._initStatus) == "undefined") {
		...
		TableObject3.prototype.init = function(params) {
			...
			if (typeof params != "object") {
				...
				alert("TableObject3.init()方法参数无效。");
			...
			if (typeof(params.tableDivId) == "string"
				...
				alert("TableObject3.init()方法中tableDivId参数必填，否则无法初始化表格。");
			...
			if (!(typeof(params.cols) == "object" && (params.cols instanceof ColsObject3))) {
				...
				alert("TableObject3.init()方法中cols参数必填，否则无法初始化表格。");
			...
			if (typeof(params.header) == "object"
				...
				// 将TableObject3对象传给TableHeaderObject对象
			...
			if (typeof(params.toolbar) == "object"
				...
				// 将TableObject3对象传给ToolBarObject对象
			...
			if (typeof(params.infoTip) == "object"
				...
				// 将TableObject3对象传给InfoTipObject对象
	...
	if (paramNum > 0) {
		...
		for (var i = 0; i < paramNum; i++) {
			...
			if ((arguments[i] instanceof ToolSeparatorObject3)
					...
					|| (arguments[i] instanceof DefaultDataToXlsBtnObject3)) {// 工具分隔符、回车符以及导出Excel按钮
		...
		obj.toHTML = function() {
			...
			if (paramArrs.length == 0) {
				...
				return "<div id=\"hx_table_toolbar3\" title=\"工具条\"></div>";
			}

			var tmpSB = new StringBuffer();

			tmpSB
					.append("<div id=\"hx_table_toolbar3\" title=\"工具条\"><table cellpadding=\"0\" cellspacing=\"0\" style=\"width:auto;height:100%;\"><tr>");

			for (var i = 0; i < paramArrs.length; i++) {

				if (paramArrs[i] instanceof ToolSeparatorObject3) {// 分隔符

					tmpSB.append("<td>").append(paramArrs[i].toHTML())
							.append("</td>");
				} else if (paramArrs[i] instanceof ToolButtonBrObject3) {// 换行

					tmpSB.append("</tr><tr>");
				} else if (paramArrs[i] instanceof DefaultDataToXlsBtnObject3) {// 导出Excel按钮
...
function PageObject3() {
	...
	obj.toHTML = function() {
		...
		tmpSB.append("<tr>");
		tmpSB
				.append("<td><button type=\"button\" id=\"hx_table_firstpage_btn3\" disabled class=\"hx_table_page_tn3 disabled_first_btn3\" title=\"第一页\"></button></td>");
		...
		tmpSB
				.append("<td><button type=\"button\" id=\"hx_table_privpage_btn3\" disabled class=\"hx_table_page_tn3 disabled_priv_btn3\" title=\"上一页\"></button></td>");
		...
		tmpSB
				.append("<td><button type=\"button\" id=\"hx_table_nextpage_btn3\" disabled class=\"hx_table_page_tn3 disabled_next_btn3\" title=\"下一页\"></button></td>");
		...
		tmpSB
				.append("<td><button type=\"button\" id=\"hx_table_lastpage_btn3\" disabled class=\"hx_table_page_tn3 disabled_last_btn3\" title=\"最后页\"></button></td>");
		...
		if (obj.viewJumpWidget) {
			...
			tmpSB
					.append("<td><span class=\"jumpPage3\">&nbsp;&nbsp;转到第<input type=\"text\" id=\"hx_table_page_text3\" disabled/>页</span></td>");
		}

		if (obj.viewPageInfoWidget) {

			tmpSB
					.append("<td width=\"100%\" align=\"right\"><span id=\"hx_table_page_info3\">共")
	...
	obj.reloadPage = function() {
		...
		if (obj.viewPageInfoWidget) {
			...
			$("#hx_table_page_info3").text("共" + obj.totalRow + "条记录，当前第"
...
function ColsObject3() {
	...
	if (arguments.length > 0 && typeof(arguments[0]) == "object") {
		...
		if (typeof(tmpColArr) != "object" && !(tmpColArr instanceof Array)) {
			...
			alert("ColsObject3对象参数格式不对，无法初始化。");
		...
		for (var i = 0; i < tmpColArr.length; i++) {
			...
			if (typeof(tmpColArr[i].alias) != "string"
				...
				alert("ColsObject3对象参数中alias没有定义或不合法。");
	...
	if (arguments.length == 1 && typeof(params) == "object") {
		...
		if (typeof(params.label) == "string" && params.label != "") {
			...
			InfoTipObject3.prototype.toHTML = function() {
				...
				return "<div id=\"hx_table_infobar_div3\" title=\"信息提示栏\">&nbsp;&nbsp;"
						+ this.label
			...
			InfoTipObject3.prototype.toHTML = function() {
				...
				return "<div id=\"hx_table_infobar_div3\" title=\"信息提示栏\"><span id=\"hx_table_infobar3\"></span></div>";
			}
		}
	} else {
	...
	obj.handle = function(toolbarObject) {
		...
		if (gridTotalRow > 0) {
			...
			for (var i = 1; i <= gridTotalRow; i++) {
				...
				if (tmpTrObj.children("#hx_check_col3").children().val() == "y") {// 是数据行
