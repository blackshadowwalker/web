...
public class userServlet extends HttpServlet {
	...
	public String list(ServletContext sc){
	...
	public String delete(ServletContext sc, HttpServletRequest req){
		...
		if(usercode!=null && !usercode.isEmpty()){
			...
			try{
				...
				sql = "delete from t_s_user where usercode=?";
	...
	public String add(ServletContext sc, HttpServletRequest req){
		...
		if(usercode!=null && !usercode.isEmpty()){
			...
			try{
				...
				sql = "select * from t_s_user where usercode=?";
				pstm = conn.prepareStatement(sql);
				pstm.setString(1, usercode);
				...
				if(rs!=null && rs.next()){
					...
					sql = "insert into t_s_user(usercode, username, password) values(?,?,?);";
					pstm = conn.prepareStatement(sql);
					pstm.setString(1, usercode);
